...
export const util = (() => {
    ...
    const countDownDate = () => {
        ...
        // Add scale class for alert effect to the container
        const containerElement = document.querySelector('.border.rounded-pill.shadow.py-2.px-4.mx-2.mb-4');
        containerElement.classList.add('alert-scale');
    
        setInterval(() => {
            const distance = Math.abs(count - (new Date()).getTime());
    
            dayElement.innerText = Math.floor(distance / (1000 * 60 * 60 * 24));
            hourElement.innerText = Math.floor((distance % (1000 * 60 * 60 * 24)) / (1000 * 60 * 60));
            minuteElement.innerText = Math.floor((distance % (1000 * 60 * 60)) / (1000 * 60));
            secondElement.innerText = Math.floor((distance % (1000 * 60)) / 1000);
        }, 1000);
    };
